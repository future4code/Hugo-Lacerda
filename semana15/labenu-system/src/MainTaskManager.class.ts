...
import InstructorsRoll from "./InstructorsRoll.class";
import MissionsRoll from "./MissionsRoll.class";

export default class MainTaskManager {
  private allInstructors: InstructorsRoll;
  private allStudents: StudentsRoll;
  private allMissions: MissionsRoll;
  public execute = () => {
    ...
    const studentsTxtFM = new FileManager("students.txt");
    const instructorsTxtFM = new FileManager("instructors.txt");
    const missionsTxtFM = new FileManager("missions.txt");
    ...
    this.allStudents = new StudentsRoll(studentsTxtFM.readFile());

    this.allInstructors = new InstructorsRoll(instructorsTxtFM.readFile());

    this.allMissions = new MissionsRoll(missionsTxtFM.readFile());

    const someStudAge = this.allStudents.getStudentAge(9);
  };

  public studentsLog = (): void => {
    this.execute();
    this.allMissions.studentsDataLog();
  };

  public instructorsLog = (): void => {
    this.execute();
